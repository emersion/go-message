...
// A Reader is either a whole message or a one of the parts in the body of a
// multipart entity.
//
// A Reader can only be consumed once: after its body is read, it can't be
// used anymore.
type Reader struct {
...
// NewReader makes a new message with the provided header and body. The entity's
...
// error that verifies IsUnknownCharset, but also returns an Reader that can
// be read.
func NewReader(header Header, body io.Reader) (*Reader, error) {
	var err error
...
// NewMultipartReader makes a new multipart message with the provided header and
...
// error that verifies IsUnknownCharset, but also returns an Reader that can
// be read.
func NewMultipartReader(header Header, parts []*Reader) (*Reader, error) {
	r := &multipartBody{
		header: header,
		parts:  parts,
	}

	return NewReader(header, r)
}
...
// also returns an Reader that can be read.
func ReadWithOptions(r io.Reader, opts *ReadOptions) (*Reader, error) {
...
func (e *Reader) MultipartReader() MultipartReader {
...
func (e *Reader) writeBodyTo(w *Writer) error {
	var err error
	if mb, ok := e.Body.(*multipartBody); ok {
...
func (e *Reader) WriteTo(w io.Writer) error {
...
type WalkFunc func(path []int, entity *Reader, err error) error
...
func (e *Reader) Walk(walkFunc WalkFunc) error {
